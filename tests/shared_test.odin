...
test_check_next_byte :: proc(t: ^testing.T) {
	...
	test_cases := []struct {
		...
		byte2:    string,
		expected: sim.Transfer_Code,
	}{
		// bits [2:5] = "000" -> ADD
		{"00000000", sim.Data_Transfer_Code.ADD},
		// bits [2:5] = "101" -> SUB
		{"00101000", sim.Data_Transfer_Code.SUB},
		// bits [2:5] = "111" -> CMP
		{"00111000", sim.Data_Transfer_Code.CMP},
	}

	for tc in test_cases {
		code: sim.Transfer_Code
		sim.check_next_byte(tc.byte2, &code)
...
test_bit_string_to_mod_field_code :: proc(t: ^testing.T) {
	...
	for tc in test_cases {
		result := sim.bit_string_to_mod_field_code(tc.input)
...
test_make_reg_to_reg :: proc(t: ^testing.T) {
	...
	bi := sim.make_reg_to_reg("10001001", "11000001", sim.Data_Transfer_Code.MOV)
	...
	testing.expectf(t, bi.code == sim.Data_Transfer_Code.MOV, "code: got %v, expected MOV", bi.code)

	// With displacement
	bi2 := sim.make_reg_to_reg("10001001", "01000001", sim.Data_Transfer_Code.ADD, displacement = u8(5))
...
test_make_immediate_with_mod :: proc(t: ^testing.T) {
	...
	bi := sim.make_immediate_with_mod("10000011", "11000001", sim.Data_Transfer_Code.ADD, data = u8(10))
...
test_make_immediate_to_reg :: proc(t: ^testing.T) {
	...
	bi := sim.make_immediate_to_reg("10110001", sim.Data_Transfer_Code.MOV, data = u8(7))
...
test_make_accumulator_immediate :: proc(t: ^testing.T) {
	...
	bi := sim.make_accumulator_immediate("00000100", sim.Data_Transfer_Code.ADD, data = u8(5))
...
test_reg_assembly_instructions_direction_0 :: proc(t: ^testing.T) {
	...
	bi := sim.ByteInstructions{
		code = sim.Data_Transfer_Code.MOV,
		byte1 = sim.Byte1{ opcode = "100010", direction = '0', word_op = '1' },
		byte2 = sim.Byte2{ mod = "11", reg = "000", rm = "001" },
	}

	ai, err := sim.reg_assembly_instructions(bi)
...
test_reg_assembly_instructions_direction_1 :: proc(t: ^testing.T) {
	...
	bi := sim.ByteInstructions{
		code = sim.Data_Transfer_Code.MOV,
		byte1 = sim.Byte1{ opcode = "100010", direction = '1', word_op = '1' },
		byte2 = sim.Byte2{ mod = "11", reg = "000", rm = "001" },
	}
...
test_reg_assembly_instructions_no_direction_rm_reg_mode :: proc(t: ^testing.T) {
	...
	bi := sim.ByteInstructions{
		code = sim.Data_Transfer_Code.MOV,
		byte1 = sim.Byte1{ opcode = "1100011", word_op = '1' },
		byte2 = sim.Byte2{ mod = "11", rm = "001" },
		data = u16(12),
	}
...
test_reg_assembly_instructions_no_direction_no_rm :: proc(t: ^testing.T) {
	...
	bi := sim.ByteInstructions{
		code = sim.Data_Transfer_Code.MOV,
		byte1 = sim.Byte1{ opcode = "1011", word_op = '0' },
		byte2 = sim.Byte2{ reg = "001" },
		data = u8(12),
	}
