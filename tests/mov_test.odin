...
test_rm_assembly_reg_mode_all_registers :: proc(t: ^testing.T) {
	...
	for tc in test_cases {
		...
		result, err := decoder.rm_assembly(bi, .REG_MODE)
...
test_rm_assembly_memory_modes :: proc(t: ^testing.T) {
	...
	for tc in base_cases {
		bi := decoder.ByteInstructions{
			byte1 = decoder.Byte1{ word_op = '1' },
			byte2 = decoder.Byte2{ rm = tc.rm },
		}

		result, err := decoder.rm_assembly(bi, .MEMORY_MODE_NO_DISPLACEMENT)
		testing.expect(t, err == nil)
		testing.expectf(t, result == tc.expected,
	...
	{
		bi := decoder.ByteInstructions{
			byte1 = decoder.Byte1{ direction = '0', word_op = '1' },
			byte2 = decoder.Byte2{ rm = "110" },
			data  = u16(4660),
		}
...

	{
		...
		result, err := decoder.rm_assembly(bi, .MEMORY_MODE_8_BIT_DISPLACEMENT)
		testing.expect(t, err == nil)
...

	{
		...
		result, err := decoder.rm_assembly(bi, .MEMORY_MODE_16_BIT_DISPLACEMENT)
		testing.expect(t, err == nil)
		testing.expect(t, result == "[BP]")
	}
...

	{
		...
		_, err := decoder.rm_assembly(bi, .UNDEFINED)
		testing.expect(t, err == decoder.DecodeErrors.Undefined_Memory_Mode)
	}
}
...
test_mov_bit_string_to_opt :: proc(t: ^testing.T) {
	...
	for tc in test_cases {
		result := decoder._mov_bit_string_to_opt(tc.input)
		testing.expectf(t, result == tc.expected,
...
test_decode_mov_regmem_with_reg :: proc(t: ^testing.T) {
	...
	incr := decoder.decode_mov("10001001", data, 0, &instructions)
	testing.expect(t, incr == 1)
	testing.expect(t, len(instructions) == 1)

	bi := instructions[0]
	testing.expect(t, bi.code == decoder.Data_Transfer_Code.MOV)
...
test_decode_mov_regmem_with_reg_bp_zero_disp8 :: proc(t: ^testing.T) {
	...
	incr := decoder.decode_mov("10001011", data, 0, &instructions)
...
test_decode_mov_immediate_to_reg :: proc(t: ^testing.T) {
...

	{
		...
		incr := decoder.decode_mov("10110000", data, 0, &instructions)
		testing.expect(t, incr == 1)
		testing.expect(t, len(instructions) == 1)
...

	{
		...
		incr := decoder.decode_mov("10111000", data, 0, &instructions)
		testing.expect(t, incr == 2)
		testing.expect(t, len(instructions) == 1)
...
test_decode_mov_immediate_to_regmem_direct_address_word :: proc(t: ^testing.T) {
	...
	incr := decoder.decode_mov("11000111", data, 0, &instructions)
...
test_decode_mov_accumulator_memory_variants :: proc(t: ^testing.T) {
...

	{
		...
		incr := decoder.decode_mov("10100000", data, 0, &instructions)
		testing.expect(t, incr == 1)
		testing.expect(t, len(instructions) == 1)
...

	{
		...
		incr := decoder.decode_mov("10100001", data, 0, &instructions)
		testing.expect(t, incr == 2)
		testing.expect(t, len(instructions) == 1)
...

	{
		...
		incr := decoder.decode_mov("10100010", data, 0, &instructions)
		testing.expect(t, incr == 1)
		testing.expect(t, len(instructions) == 1)
...

	{
		...
		incr := decoder.decode_mov("10100011", data, 0, &instructions)
		testing.expect(t, incr == 2)
		testing.expect(t, len(instructions) == 1)
